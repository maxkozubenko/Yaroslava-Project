...
const getRandomIntInclusive = (min, max) => {
  ...
  const postLoad = async (req, res) => {
    try {
        if(!req.params.id) {
          return res.status(200).send({message: 'Please specify id'})
        }
        const load = await Load.findOne({_id: req.params.id});
        if(!load) {
            return res.status(200).send({message: 'No load found'});
        }
        ...
        if(load.status !== 'NEW') {
            return res.status(200).send({message: 'You can post only a new load'});
        }
        await Load.findByIdAndUpdate(req.params.id, {status: 'POSTED'});
        const suitableTrucks = await Truck.find({status: 'IS'});
        if(!suitableTrucks) {
            return res.status(200).send({message: 'Unfortunately there are no available trucks'});
        }
        ...
        if(readyToDeliverTrucks.length === 0) {
            await Load.findByIdAndUpdate(req.params.id, {status: 'NEW', logs: {message: 'Load could not be assigned as there was no available truck', time: new Date().toISOString()}})
                .then(() => res.status(200).send({message: 'Load cannot be assigned as there is no available driver at the moment'}))
                .catch(() => res.status(400).send({message: 'Client error'}));
  ...
  const getLoads = async (req, res) => {
    try {
        ...
        if(user.role === 'DRIVER') {
            await Load.find({status: 'ASSIGNED' || 'SHIPPED', assigned_to: user._id})
                .skip(+offset || 0)
                .limit(+limit || 0)
